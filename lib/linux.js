...
function findChromiumExecutablesForLinuxDesktop(folder) {
  const argumentsRegex = /(^[^ ]+).*/; // Take everything up to the first space
  const chromiumExecRegex = '^Exec=\/.*\/(google|chromium)-.*';
  ...
  if (canAccess(folder)) {
    ...
    let execPaths;
    execPaths = execSync(`find "${folder}" -type f -exec grep -E "${chromiumExecRegex}" "{}" \\; | awk -F '=' '{print $2}'`);
...
function linux() {
  ...
  desktopInstallationFolders.forEach(folder => {
    installations = installations.concat(findChromiumExecutablesForLinuxDesktop(folder));
  });
  ...
  executables.forEach((executable) => {
    ...
    const validChromiumPaths = [
      ...
      try {
        const chromiumPathToTest = possiblePath + '/' + executable;
        if (fs.existsSync(chromiumPathToTest) && canAccess(chromiumPathToTest) && isExecutable(chromiumPathToTest)) {
          installations.push(chromiumPathToTest);
          return chromiumPathToTest;
        }
      } catch (err) {
        // not installed on this path or inaccessible
      }
      return undefined;
    }).filter((foundChromiumPath) => foundChromiumPath);

    // skip asking "which" command if the binary was found by searching the known paths.
    if (validChromiumPaths && validChromiumPaths.length > 0) {
      return;
    }
